...
console.show();
//console.setSize(2000, device.height);
var start='[]'
 applist=eval('(' + start + ')'); 
var tempstr="";
var appname="";
            ...
            if(r.statusCode=="200"){  
             ...
             //   log(jsonstr);
                try{
                    ...
                    if(result==null){
                        log("Error:"+appname+" ver:"+appver+" 未安装");
                    }else{
                        var localappver=thiscommon.getPackageVersion(pname);
                        //log("local ver is:"+localappver+" server ver is:"+appver);
                        if(localappver!=appver){
                      //   log(appname);
                               log("Error:"+appname+" 版本不匹配"+" 云端版本:"+appver+" 本地版本:"+localappver)
                        }


                    
                    }
                  //  alert("appname is:"+tempjson['appname']+"\npackagename is:"+tempjson['packagename']+"\n appver is:"+tempjson['appver']);
                  //  exit();
                            //将json添加到applist中
...
console.show();
log('im alive')
var myUrl = "http://192.168.31.250/appname/";
